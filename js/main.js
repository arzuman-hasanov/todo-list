...
sortAsc.addEventListener("click",(e)=>{
    ...
    while(switching){
        ...
        if(listitems[i+1] != undefined){
            listitems[i].parentNode.parentNode.insertBefore(listitems[i+1].parentNode,listitems[i].parentNode);
            switching = true;
        }
...
search.addEventListener('keyup',()=>{
    ...
    arr.forEach(item=>{
        if( !item.firstChild.value.toLowerCase().trim().includes(search.value.toLowerCase().trim())){
           item.classList.add('hide');
        }else if(item.firstChild.value.toLowerCase().trim().includes(search.value.toLowerCase().trim())){
